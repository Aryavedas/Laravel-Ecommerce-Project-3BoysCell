...
<body class="font-sans antialiased">
    <div class="min-h-screen bg-gray-100">
        ...
        <main>
            {{-- {{ $slot }} --}}
            <div
                class="container border bg-white py-7 mx-auto rounded-lg shadow-sm my-6 lg:my-10 px-3 md:px-6 lg:px-10">

                <div class="w-full flex flex-col md:flex-row justify-between items-center pb-7 gap-4 md:gap-0">
                    <h1 class="font-extrabold text-lg md:text-xl lg:text-2xl text-center md:text-left">Selamat Datang
                        Ditoko Kami</h1>
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-5">
                    ...
                    @forelse ($barangs as $barang)
                        <div class="card bg-white border shadow-sm p-2 md:p-3 rounded-lg flex flex-col h-full">
                            ...
                            <div
                                class="card-img w-full h-[150px] md:h-[220px] lg:h-[300px] overflow-hidden rounded-md relative">
                                {{-- Tambahkan fallback jika gambar null --}}
                                @if ($barang->gambar)
                                    {{-- PERBAIKAN DISINI: Menambahkan disk('s3') --}}
                                    <img class="object-cover object-center h-full w-full hover:scale-105 transition-transform duration-300"
                                        src="{{ Storage::disk('s3')->url($barang->gambar) }}" alt="{{ $barang->nama }}"
                                        onerror="this.onerror=null; this.src='https://via.placeholder.com/300?text=Gambar+Rusak';">
                                @else
                                    <img class="object-cover object-center h-full w-full bg-gray-200"
                                        src="https://via.placeholder.com/300?text=No+Image" alt="No Image">
                                @endif
                            </div>
                            ...
                            <div class="card-body flex flex-grow flex-col p-1.5 md:p-2.5">
                                ...
                                <div class="card-title my-2">
                                    <h1
                                        class="font-extrabold text-sm md:text-xl lg:text-2xl text-sky-700 leading-tight">
                                        {{ $barang->nama }}</h1>
                                </div>

                                <div class="card-description my-1 md:my-3">
                                    <p class="line-clamp-2 md:line-clamp-3 text-slate-700 text-xs md:text-base">
                                        {{ $barang->deskripsi }}</p>
                                </div>
